...
public class SVGProcessor {
	...
	private Line maxHorizontalLine;
	...
	public static String[] openFile(String p)throws IOException{
		...
		String [] file = new String[temp.size()];
...
//		System.out.println(true);
	...
	public void draw(){
		...
		for (int i = 0; i < actualPoints.size(); i++) {
			s +="<path class=\"point\" d=\"m"+
					(Math.floor(actualPoints.get(i).getX())-5)+" "+
					(Math.floor(actualPoints.get(i).getY()))+
...
//		s += drawActualHorizontalLines();
		...
		s += drawSwitch();
		try {
			String [] html = openFile("C:/Users/omar/Desktop/bachlorVideo/index4.html");
			PrintWriter writer2 = new PrintWriter("C:/Users/omar/Desktop/bachlorVideo/index5.html", "UTF-8");
			for (int i = 0; i < html.length; i++) {
				writer2.println(html[i]);
				if(containString(html[i],"img")){
					for (int j = 0; j < file.length; j++) {
						
						writer2.println(file[j]);
						if(j == file.length-2){
							writer2.println(s);
						}
					}
...
//			PrintWriter writer = new PrintWriter("C:/Users/omar/Desktop/bachelor/output.svg", "UTF-8");
	...
	public static boolean containString(String big , String small){
		for (int i = 0; i < big.length(); i++) {
			if(big.charAt(i) == small.charAt(0)){
				try{
					for (int j = 1; j < small.length(); j++) {
...
//				System.out.println(new Line(firstCorner,secondCorner).length());
	...
	public String drawVerticalLines(){
		...
		for (int i = 0; i < verticalLines.size(); i++) {
			...
			s+="<path class=\"verLine\" d=\"m"+(int)(Math.ceil(verticalLines.get(i).getP1().getX()))+" "+
...
//			}
			...
			s+="<path class=\"horLine\" d=\"m"+(int)(Math.ceil(horizontalLines.get(i).getP1().getX()))+" "+
...
//		return HpowerSourcePoints;
	...
	public String drawCornerPoints(){
		...
		for (int i = 0; i < cornerPoints.size(); i++) {
			s +="<path class=\"corPoint\" d=\"m"+(Math.floor(cornerPoints.get(i).getX())-5)+" "+(Math.floor(cornerPoints.get(i).getY()))+
					" l0 5 l10 0 l0 -10 l-10 0 l0 5z\" fill=\"blue\"/>";
		}
		return s;
	}
...
//				System.out.println("after "+verticalLines.size());
	...
	public String drawNodes(){
		...
		for (int i = 0; i < nodes.size(); i++) {
			s +="<path class=\"node\" d=\"m"+(Math.floor(nodes.get(i).getX())-5)+" "+(Math.floor(nodes.get(i).getY()))+
	...
	public void setMaxHorizontalLength(){
		...
		for (int i = 0; i < horizontalLines.size(); i++) {
			if(horizontalLines.get(i).length() > maxSoFar){
				maxHorizontalLine = horizontalLines.get(i);
	...
	public String drawValues(){
		...
		for (int i = 0; i < resistors.size(); i++) {
			...
			if(orientation == 0){
				...
				s += "<text class=\"value\" font-size=\"40\" x=\""+midx+"\" y=\""+height+"\" fill=\"red\">"+
...
//					height = resistors.get(i).getStartPoint().getX()-thicknessOfLines*16;
				...
				s += "<text class=\"value\" font-size=\"40\" x=\""+height+"\" y=\""+midy+"\" fill=\"red\">"+
						String.format("%.2f",resistors.get(i).getVoltageValue())+" V , "+
						String.format("%.2f",resistors.get(i).getCurrentValue())+" A</text>";
			}
		}
...
//		}
		...
		for (int i = 0; i < temp2.size()-1; i++) {
			s += "<polyline class=\"current\" points=\"";
...
//		System.out.println(getAnglesInComponent(components.get(0)));
	...
	public boolean matchResistors(Component c) throws IOException{
		...
		for (int i = 0; i < resistorsPaths.size(); i++) {
			...
			resistor.file = openFile(resistorsPaths.get(i));
			resistor.setOrigin();
...
//		}
	...
	public String drawComponentsBoundries(){
		String s = "";
		for (int i = 0; i < componentsBoundries.size(); i++) {
			s +="<path class=\"boundry\" d=\"m"+(Math.floor(componentsBoundries.get(i).getX())-5)+" "+(Math.floor(componentsBoundries.get(i).getY()))+
					" l0 5 l10 0 l0 -10 l-10 0 l0 5z\" fill=\"orange\"/>";
		}
		return s;
	}
...
//		System.out.println("components size: "+components.size());
		for (int i = 0; i < components.size(); i++) {
			s +="<path class=\"boundry\" d=\"m"+(Math.floor(components.get(i).getBoundry1().getX())-5)+" "+(Math.floor(components.get(i).getBoundry1().getY()))+
					" l0 5 l10 0 l0 -10 l-10 0 l0 5z\" fill=\"orange\"/>";
			s +="<path class=\"boundry\" d=\"m"+(Math.floor(components.get(i).getBoundry2().getX())-5)+" "+(Math.floor(components.get(i).getBoundry2().getY()))+
					" l0 5 l10 0 l0 -10 l-10 0 l0 5z\" fill=\"orange\"/>";
			s +="<path class=\"boundry\" d=\"m"+(Math.floor(components.get(i).getBoundry3().getX())-5)+" "+(Math.floor(components.get(i).getBoundry3().getY()))+
					" l0 5 l10 0 l0 -10 l-10 0 l0 5z\" fill=\"orange\"/>";
			s +="<path class=\"boundry\" d=\"m"+(Math.floor(components.get(i).getBoundry4().getX())-5)+" "+(Math.floor(components.get(i).getBoundry4().getY()))+
					" l0 5 l10 0 l0 -10 l-10 0 l0 5z\" fill=\"orange\"/>";
		}
		return s;
	}
	
	public String drawSwitch(){
		double length =0;
		String s = "";
		Line maxLine = new Line();
		for (int i = 0; i < lines.size(); i++) {
			if(appEqual(numberInString(lines.get(i).slope()), 0, 0.2)
					&& lines.get(i).length() > length){
				length = lines.get(i).length();
				maxLine = lines.get(i);
			}
		}
		int ExistLineBelow = 0;
		for (int i = 0; i < lines.size(); i++) {
			if(appEqual(numberInString(lines.get(i).slope()), 0, 0.2) &&
					appEqual(lines.get(i).getP1().getY(),
					maxLine.getP1().getY(),thicknessOfLines+2)){
				if(lines.get(i).getP1().getY() >
				maxLine.getP1().getY()+5){
					s += drawPoint(lines.get(i).getP1());
					ExistLineBelow = 1;
					break;
				}
			}
		}
		double switchLineLength = length/3;
		Point start = getMinX(maxLine.getP1(),maxLine.getP2());
		start.setX(start.getX()+length/3);
		Point corner1 = new Point((start.getX()+length/3)-5,start.getY());
		Point corner2;
		Point corner3;
		if(ExistLineBelow == 0){
			corner2 = new Point(corner1.getX(),corner1.getY()-thicknessOfLines);
			corner3 = new Point(start.getX(),start.getY()-thicknessOfLines);
		}
		else{
			corner2 = new Point(corner1.getX(),corner1.getY()+thicknessOfLines);
			corner3 = new Point(start.getX(),start.getY()+thicknessOfLines);
		...
		s +="<path id=\"switch\" class=\"switchLine\" d=\"m"+start.getX()+" "+start.getY()+
				" l"+(corner1.getX()-start.getX())+" "+
				(corner1.getY()-start.getY())+" l"+
				(corner2.getX()-corner1.getX())+
				" "+(corner2.getY()-corner1.getY())+" l"+
				(corner3.getX()-corner2.getX())+
				" "+(corner3.getY()-corner2.getY())+"z\" fill=\"black\"/>";
//		double hyp = switchLineLength-5;
//		double side1 = Math.abs(((hyp*Math.sin(65)))/Math.sin(90));
		double side1 = switchLineLength+5;
		double yCom = Math.abs((side1*Math.sin(65))/Math.sin(90));
//		double xCom = Math.sqrt(side1*side1 - yCom*yCom);
		double xCom = Math.abs((side1*Math.sin(25))/Math.sin(90));
		double hyp = Math.abs((side1*Math.sin(90))/Math.sin(65));
		
		s +="<path id=\"switch2\" class=\"switchTriangle\" d=\"m"+(start.getX()+hyp)
				+" "+(start.getY())+
				" l"+(-1*hyp)+" "+
				(0)+" l"+
				xCom+" "+yCom+"z\" fill=\"grey\"/>";
//		Point temp = new Point(start.getX()+hyp,start.getY());
//		s += drawPoint(temp);
		return s;
	}
	
	public String drawPoint(Point p){
		String s = "";
		s +="<path class=\"point\" d=\"m"+(p.getX()-5)+" "+
		(p.getY())+" l0 5 l10 0 l0 -10 l-10 0 l0 5z\" fill=\"pink\"/>";
		return s;
	}
...
//		System.out.println("resistors points "+resistorPoints);
	...
	public static void main(String[] args) {
		try{
			...
			SVGProcessor file = new SVGProcessor(fileName);
...
//			System.out.println(total);
		 ...
		 try {
			 Runtime.getRuntime().exec(new String[]{"cmd", "/c","start chrome file:///C:/Users/omar/Desktop/bachlorVideo/index5.html"});
        }catch (Exception e) {
        	 e.printStackTrace();
        }
	}
